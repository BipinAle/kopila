package com.mobile.kopila.utils;

public class Constants {

    public static final String SHARED_PREF = "DAUNGHA";
    public static final String AGREEMENT = "agreement";
    public static final String HAMRO_BAREMA = "harmo barema";
    public static final String DAUNGHA_KO_CHINARI = "daungha ko chinari";
    public static final String PHONE_DIARY = "phone diary";
    public static final String JANKARI = "jankari";
    public static final String PHOTO_GALLERY = "photo gallery";
    public static final String SAHAYOG_SAMPARKA = "sahayog samparka";
    public static final String NAKSHA = "naksha";

    public static final String AGREEMENT_GID = "979384947";
    public static final String HAMROBAREMA_GID = "676499527";
    public static final String DAUNGHA_KO_CHINARI_GID = "602265046";
    public static final String PHONE_DIARY_GID = "249309699";
    public static final String JANKARI_GID = "536135523";
    public static final String PHOTO_GALLERY_GID = "779910483";
    public static final String SAHAYOG_SAMPARKA_GID = "1748407582";
    public static final String NAKSHA_GID = "351744118";

}
